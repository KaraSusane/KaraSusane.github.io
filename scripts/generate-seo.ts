...
const staticShell = (content: string) => `
  <div style="max-width:960px;margin:0 auto;padding:120px 24px 72px;font-family:Arial,sans-serif;color:#1d1d1f">
    ${content}
  </div>`;
...
const standardPages: PageDefinition[] = [
  ...
  { path: '/blog', title: 'Blog prawny', description: 'Praktyczne artykuły o pismach, umowach i codziennych sytuacjach prawnych.', staticContent: staticShell(`<h1>Blog prawny</h1>${blogPosts.map((post) => `<h2><a href="/blog/${escapeHtml(post.slug)}">${escapeHtml(post.title)}</a></h2><p>${escapeHtml(post.excerpt)}</p>`).join('')}`) },
